...
export function renderPdfBuffer(doc) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const labour = Number(doc.labourCost || 0);
    const materials = Number(doc.materialsCost || 0);
    ...
    if (labour > 0) row("Labour", money(labour));
    if (materials > 0) row("Materials", money(materials));
    row("Subtotal", money(doc.subtotal));
    if (discount > 0) row("Discount", `- ${money(discount)}`);
    row(`VAT (${Math.round(doc.taxRate * 100)}%)`, money(doc.taxAmount));
    ...
    pdf.fontSize(10);
    if (doc.notes) {
      pdf.fillColor("#0f172a").text("Notes", 50, y);
      pdf.fillColor("#64748b").text(doc.notes, 50, y + 14, { width: 495 });
      y += 14 + pdf.heightOfString(doc.notes, { width: 495 }) + 12;
    }
    if (doc.paymentTerms) {
      pdf.fillColor("#0f172a").fontSize(10).text("Payment terms", 50, y);
      pdf
        .fillColor("#64748b")
        .text(doc.paymentTerms, 50, y + 14, { width: 495 });
    }
    pdf
      .fillColor("#94a3b8")
      .fontSize(9)
      .text(`Thank you for choosing ${company.name}.`, 50, 792, {
        align: "center",
        width: 495,
      });
